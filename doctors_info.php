...
<body>
                                ...
                                <tr>
                                    ...
                                    <!-- <td id="message-<?= $faculty['id'] ?>">
                                                ...
                                                if ($wordCount > 10) {
                                                    ...
                                                    echo '<a href="#" class="btn btn-link" onclick="showFullMessages(' . $faculty['id'] . ')">Read More</a>';
                                                } else {
                                    ...
                                    <td id="messages-<?= $faculty['id'] ?>" class="message-container">
                                        <?php
                                        $wordCount = str_word_count($faculty['message']);
                                        $shortMessage = limitWords($faculty['message']);
                                        if ($wordCount > 10) {
                                            // If the message exceeds 10 words, show the limited version initially
                                            echo '<span class="short-messages">' . $shortMessage . '</span>';
                                            echo '<span class="full-messages" style="display:none;">' . $faculty['message'] . '</span>';
                                            echo '<a href="javascript:void(0)" class="btn btn-link read-more" onclick="showFullMessages(' . $faculty['id'] . ')">Read More</a>';
                                        } else {
                                            // If the message does not exceed 10 words, show the full message
                                            echo $faculty['message'];
                                        }
    ...
    <script>
        ...
        function showFullMessages(facultyId) {
            var messageContainer = document.getElementById('messages-' + facultyId);
            var shortMessage = messageContainer.querySelector('.short-messages');
            var fullMessage = messageContainer.querySelector('.full-messages');
            var readMoreLink = messageContainer.querySelector('.read-more');
            ...
            fullMessage.style.display = 'inline';
            readMoreLink.textContent = 'Read Less'; // Change link text to 'Read Less'
            readMoreLink.setAttribute('onclick', 'showLessMessages(' + facultyId + ')'); // Change link to toggle back
        }
        ...
        function showLessMessages(facultyId) {
            var messageContainer = document.getElementById('messages-' + facultyId);
            var shortMessage = messageContainer.querySelector('.short-messages');
            var fullMessage = messageContainer.querySelector('.full-messages');
            var readMoreLink = messageContainer.querySelector('.read-more');
